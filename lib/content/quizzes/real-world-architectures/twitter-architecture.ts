...
export const twitterarchitectureQuiz = [
    ...
    {
        id: 'q3',
        question: 'Describe Twitter\'s real-time search system (Earlybird). How does it index tweets within seconds for billions of searches?',
        sampleAnswer: 'Earlybird is Twitter\'s real-time search engine built on Lucene. Indexing: (1) User tweets. (2) Tweet stored in Manhattan (database). (3) Published to Kafka. (4) Earlybird consumes Kafka, indexes tweet. (5) Tweet searchable within 5 seconds. Index structure: Inverted index (keyword → tweet IDs), sharded by time (recent tweets = hot shard). Search query: User searches "machine learning" → Query all shards, retrieve matching tweet IDs, rank by relevance + recency + engagement. Ranking: ML model considers keyword relevance (BM25), recency (newer boosted), engagement (likes, retweets), social graph (followed accounts boosted). Challenges: 500M tweets/day to index, billions of searches, trending topics spike traffic. Solutions: Horizontal sharding (100s of shards), time-based sharding (old tweets archived), caching popular queries.',
        keyPoints: [
