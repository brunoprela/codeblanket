export const derivativeRiskManagementQuiz = [
  ...
  },
  {
    id: 'drm-q-3',
    question:
      'Implement a real-time risk limit monitoring system that: (1) Tracks Greeks (DV01, delta, gamma, vega) across all positions, (2) Enforces hard limits (reject trades if exceeded), (3) Generates alerts at threshold levels (80% of limit), (4) Aggregates exposure by desk, region, asset class, (5) Produces daily risk reports. Include: pre-trade checks, post-trade reconciliation. How do you handle: limit breaches (force unwind vs exception approval)? Intraday monitoring vs end-of-day? Limit utilization optimization?',
    sampleAnswer:
      'Limit monitoring: Real-time Greeks: Calculate on every trade, aggregate by hierarchy (trader → desk → region), Hard limits: Pre-trade check, reject if limit exceeded (API returns 400), Alerts: Email/SMS at 80%, 90%, 100% utilization, Exposure aggregation: Sum DV01 across all bonds/swaps, sum delta across all options/equity, Reporting: Daily summary (current, limit, utilization %), breaches flagged, Breach handling: Automatic rejection (hard limit), or escalate to CRO for approval (soft limit).',
    keyPoints: [
