export const multi_region_deploymentQuiz = {
  title: 'Quiz - Multi Region Deployment',
  questions: [
    {
      question: `You're scaling a production LLM application related to multi region deployment. You've identified a critical bottleneck that's causing performance issues. Analyze the root cause, explain why this is a common problem in scaled systems, && design a comprehensive solution with specific implementation details, monitoring strategies, && expected performance improvements.`,
      answer: `[This answer would be 300-400 words covering:]
      
**Root Cause Analysis**:
- Identify the specific bottleneck in the multi region deployment context
...
- Operational complexity changes`,
    ...
    {
      question: `Compare && contrast two different approaches to multi region deployment (e.g., Strategy A vs Strategy B). Include specific use cases where each approach excels, quantitative comparisons of cost/performance/complexity, && provide a decision framework for choosing between them in production.`,
...
Application X chose Strategy A because [reasons], resulting in [outcomes]`,
    ...
    {
      question: `Design a monitoring && optimization strategy for multi region deployment in a production LLM application serving 100,000 users. Include specific metrics to track, alert thresholds, optimization opportunities to look for, && a process for continuous improvement. Explain how you would measure ROI of optimizations.`,
      answer: `[This answer would be 300-400 words covering:]
...
3. **Quality Metrics**:
   - Success rate
   - User satisfaction
   - Specific to multi region deployment
