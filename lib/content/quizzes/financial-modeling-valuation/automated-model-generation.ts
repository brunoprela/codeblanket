export const automatedModelGenerationQuiz = [
  ...
  {
    id: 'amg-q-3',
    question: 'Your automated platform values 500 companies daily. How do you identify which valuations to trust vs which need manual review?',
    sampleAnswer: 'Automated quality scoring: Assign confidence score (0-100) based on: (1) Data completeness (100 if all fields populated, 0 if missing revenue). (2) Model fit (100 if R² > 0.8 in projections, lower if poor fit). (3) Valuation reasonableness (flag if 3x current price or <0.3x). (4) Consistency (flag if swings >50% day-over-day). Auto-route: Score >80: Auto-approve, publish. Score 60-80: Flag for junior analyst review. Score <60: Flag for senior analyst deep dive. Example: Score = (Data complete)×0.3 + (Model R²)×0.3 + (Valuation reasonable)×0.2 + (Consistency)×0.2. Only manually review 20% flagged, automate 80%.',
    keyPoints: [
