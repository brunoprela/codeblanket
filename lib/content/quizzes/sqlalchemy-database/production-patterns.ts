export const productionPatternsQuiz = [
  {
    id: 'sql-prod-q-1',
    question:
      'Design a complete production database configuration for a high-traffic web application (10K requests/second). Address: (1) connection pool sizing, (2) caching strategy, (3) monitoring setup, (4) error handling, (5) health checks. Include complete code examples.',
    sampleAnswer:
...
export const productionPatternsMultipleChoice: MultipleChoiceQuestion[] = [
  {
    ...
    correctAnswer: 1,
    explanation:
      `pool_pre_ping=True tests connection before checkout. ` +
      `Problem: Connections can die (network issue, database restart, idle timeout). ` +
      `Without pre_ping: Application gets dead connection, query fails with error. ` +
      `With pre_ping: Engine tests connection (SELECT 1), if dead, creates new connection, application gets working connection. ` +
      `Small overhead (extra ping), but prevents errors. Critical for production reliability. ` +
  ...
  },
  {
    ...
    explanation:
      `pool_size = number_of_application_threads. ` +
      `Reasoning: Each thread needs one connection when executing query. ` +
      `Example: 10 gunicorn workers * 4 threads = 40 threads → pool_size=40. max_overflow = 2 * pool_size for burst capacity. ` +
      `Too large: Wastes DB resources, connections idle. ` +
      `Too small: Threads wait for connections, pool_timeout errors. ` +
      `Formula: pool_size + max_overflow < database_max_connections. ` +
      `PostgreSQL default max_connections=100, so pool_size=40 + max_overflow=40 = 80 (safe).`,
  },
  {
    id: 'sql-prod-mc-3',
  ...
  },
  {
    ...
    correctAnswer: 1,
    explanation:
      'The Repository pattern abstracts data access behind a consistent interface. ' +
      'Benefits include: (1) Testability—by using mock repositories in unit tests, you avoid needing a real database. ' +
      '(2) Swappability—easily change your database technology (e.g., replacing SQLAlchemy with MongoDB) without rewriting business logic. ' +
      '(3) Organization—centralizes all user-related queries within a UserRepository class. ' +
      '(4) Clean architecture—business logic depends only on repository interfaces, not on implementation details like SQLAlchemy. ' +
      'For example: The UserService depends on the UserRepository interface. For unit tests, you can inject a mock repository; for integration tests, use a real SQLAlchemy repository. ' +
      'In production, repositories enable maintainable design for large and complex domain-driven applications.',
  },
  {
    id: 'sql-prod-mc-5',
    ...
    correctAnswer: 1,
    explanation:
      'A database health check endpoint should: (1) Test database connectivity by executing a simple query (like SELECT 1); if this fails, the database is likely down or unreachable. ' +
      '(2) Check the connection pool status—compare checked_out connections to pool size; if more than 80% are in use, the pool is nearly exhausted and performance may degrade. ' +
      'If healthy, the endpoint should return HTTP 200 OK; if unhealthy, it should return HTTP 503 Service Unavailable. ' +
      'Load balancers depend on this endpoint to route traffic only to healthy application instances. ' +
      'Example: If Instance A’s health check fails (DB down), it returns 503 and the load balancer stops sending requests to it. Instance B (healthy) returns 200, so the load balancer sends all requests to B. ' +
      'Good health checks are critical for zero-downtime deployments and automatic failover.',
  },
];
