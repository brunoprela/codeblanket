export const dataNormalizationQuiz = [
  ...
  },
  {
    ...
    question:
      'You receive quotes from 3 exchanges with timestamps: NASDAQ 10:30:00.123456, NYSE 10:30:00.125789 (+2.3ms), ARCA 10:30:00.121000 (-2.4ms). Clocks are not synchronized. How to: (1) Detect clock skew, (2) Order quotes correctly, (3) Calculate accurate NBBO, (4) Handle in production?',
    sampleAnswer:
      "Clock skew handling: (1) Detection: Compare exchange timestamps to atomic reference (GPS, PTP). If NYSE consistently 2ms ahead, that's clock skew. Monitor drift over time. (2) Ordering: Don't trust absolute timestamps. Use sequence numbers per exchange (monotonic ordering). For cross-exchange ordering, apply known offsets (NYSE-2ms). (3) NBBO: Calculate per-exchange BBO first, then combine. Don't mix timestamps across exchanges. (4) Production: Sync system clock with NTP/PTP. Log timestamp discrepancies. Alert if skew > 10ms. Use sequence numbers as primary ordering mechanism.",
    keyPoints: [
