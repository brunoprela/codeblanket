...
`,
  exercises: [
    {
      prompt: "Build an ETF premium/discount monitor that fetches real-time ETF prices and NAVs, calculates premium/discount %, alerts when >0.5% deviation occurs, and tracks historical patterns. Monitor SPY, QQQ, IWM, and sector ETFs.",
      solution: "// Implementation: 1) Connect to market data API (Alpha Vantage, IEX), 2) Fetch ETF market prices (real-time or 15-min delayed), 3) Scrape NAV from ETF provider websites (State Street for SPY, etc.), 4) Calculate (Price - NAV) / NAV * 100, 5) Store in database with timestamp, 6) Alert if abs(premium/discount) > 0.5%, 7) Create dashboard showing current and historical premium/discount charts, 8) Identify patterns (ETFs trading at consistent discounts = buy opportunity)"
    },
    {
      prompt: "Create a factor ETF backtesting system that compares Value (VTV), Momentum (MTUM), Quality (QUAL), Low Vol (USMV), and Size (IWM) ETFs over 10 years. Calculate returns, volatility, Sharpe ratios, max drawdowns, and optimal factor allocations using Markowitz optimization.",
      solution: "// Implementation: 1) Fetch 10 years of daily prices for factor ETFs using yfinance/Alpha Vantage, 2) Calculate daily/monthly returns, 3) Compute metrics: CAGR, annual volatility, Sharpe ratio, max drawdown, 4) Calculate correlation matrix, 5) Implement Markowitz optimization to find max Sharpe portfolio, 6) Backtest equal-weight, market-cap-weight, and optimized portfolios, 7) Visualize cumulative returns, drawdowns, rolling Sharpe ratios, 8) Compare factor timing vs buy-and-hold"
    }
  ]
};
