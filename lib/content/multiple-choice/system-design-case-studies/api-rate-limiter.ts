...
export const rateLimiterMultipleChoice = [
  {
    ...
    question:
      "A user makes 80 requests in the previous window (12:00:00-12:00:59) and 40 requests in the current window (12:01:00-12:01:59). At 12:01:45, should the user's 121st total request be allowed if the limit is 100 requests/minute using sliding window counter algorithm?",
    options: [
  ...
  },
  {
    ...
    question:
      'You implement rate limiting in Redis with these commands: `GET rate_limit:user:123`, check if under limit, then `INCR rate_limit:user:123`. Under high concurrency (10,000 requests/second for user 123 with a 100 req/sec limit), what is the most likely outcome?',
    options: [
  ...
  },
  {
    ...
    question:
      'Your rate limiter uses Redis with a 60-second window. A key is set as `SETEX rate_limit:user:123:1672531200 120 50`. Why is the TTL set to 120 seconds (2× window size) instead of 60 seconds?',
    options: [
  ...
  },
  {
    ...
    question:
      'Your API rate limiter returns HTTP 429 with header `Retry-After: 45`. A client implements exponential backoff: retry after 1s, 2s, 4s, 8s, 16s... What is the problem with this approach?',
    options: [
  ...
  },
  {
    ...
    question:
      "You're implementing a token bucket rate limiter with capacity=100 tokens and refill_rate=1.67 tokens/second (100 tokens/minute). At time T=0, the bucket is full (100 tokens). A client makes 100 requests instantly at T=0, consuming all tokens. When will the bucket be full again?",
    options: [
