...
export const ticketmasterMultipleChoice = [
    {
        question: "When implementing seat holds in Ticketmaster, you use Redis with `SETEX seat:A1:hold user_123 600` to reserve a seat for 10 minutes. The user completes payment after 8 minutes. What is the correct cleanup procedure to release the Redis lock?",
        options: [
    ...
    },
    {
        ...
        correctAnswer: 2,
        explanation: "The system should decrease the admission rate to 500 users/minute because two critical metrics are showing stress: database CPU at 85% (target < 80%) and API latency p95 at 800ms (target < 500ms). These metrics indicate the system is approaching capacity limits, and admitting more users would risk cascading failures—high latency leads to connection timeouts, retries amplify load, and the system could crash. Option A (increase rate) would worsen the problem by adding more load to an already stressed system. Option B (maintain rate) ignores warning signs; the metrics show degradation, not stability. Option D (stop entirely) is overly aggressive—a gradual decrease to 500 users/minute reduces load while still processing the queue. The dynamic rate controller should continuously monitor these metrics and adjust admission rates accordingly: decrease by 50% if metrics exceed thresholds, increase by 50% if metrics are healthy (CPU < 60%, latency < 300ms). This provides backpressure without completely halting service."
    },
    {
        question: "Two users (User A and User B) simultaneously try to purchase seat 'A1' at exactly the same millisecond. The system uses Redis distributed locks with `SET seat:A1:lock NX EX 600`. What is the guaranteed outcome with this approach?",
        options: [
