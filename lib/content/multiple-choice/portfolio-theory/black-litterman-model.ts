export const blackLittermanModelMC = {
    ...
    questions: [
        ...
        },
        {
            ...
            correctAnswer: 1,
            explanation: 'Answer: B. Confidence is encoded in Omega matrix: Ω = τP∑P^T where τ controls uncertainty. Higher confidence → lower τ → tighter uncertainty → posterior closer to view. Typical: 90% confidence ≈ τ=0.025, 50% confidence ≈ τ=0.5. Not a simple probability (A), not a t-statistic (C), not analyst count (D - though that might inform confidence qualitatively).'
        },
        {
            id: 'bl-mc-3',
            type: 'multiple-choice' as const,
            question: 'Market cap weights of 70% US, 30% International imply (with λ=2.5, correlations given):',
            options: [
        ...
        },
        {
            ...
            correctAnswer: 1,
            explanation: 'Answer: B. BL combines equilibrium returns (Π from market) with investor views (Q) using Bayesian update. Formula weights by inverse uncertainty: high confidence view gets more weight, low confidence view gets less. Result closer to equilibrium if uncertain, closer to view if confident. Not historical/analyst (A), not CAPM components (C), not time horizons (D). The Bayesian blending is the key BL innovation.'
        }
    ]
};
