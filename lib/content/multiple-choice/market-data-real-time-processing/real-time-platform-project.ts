...
export const realTimePlatformProjectMultipleChoice: MultipleChoiceQuestion[] = [
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation:
      'SLA calculation: 99.9% uptime = 0.1% downtime allowed. Per year: 365 days × 24 hours = 8760 hours total. Downtime: 8760 × 0.001 = 8.76 hours/year = 525.6 minutes. Per month: 8.76 / 12 = 43.8 minutes. Common SLAs: 99% = 3.65 days/year. 99.9% = 8.76 hours/year (three nines). 99.99% = 52.6 minutes/year (four nines). 99.999% = 5.26 minutes/year (five nines, extremely expensive). For trading: 99.9% minimum (market hours = 6.5 hrs/day, cannot afford daily outages).',
  },
  {
    id: 'real-time-platform-project-mc-4',
    question:
      'Load test shows P99 latency = 15ms (target < 10ms). Where to optimize first?',
    options: [
      'Identify slowest component via distributed tracing',
      'Add more servers',
      'Upgrade CPU',
      'Reduce data volume',
    ],
    correctAnswer: 0,
    explanation:
      'Optimization approach: (1) Measure: Use distributed tracing (Jaeger/Zipkin) to find bottleneck. Latency breakdown: Ingestion 2ms + Normalization 5ms + Kafka 7ms + Consume 1ms = 15ms. Bottleneck: Kafka (7ms). (2) Optimize: Reduce Kafka latency (linger_ms=0, compression=none, local broker). Result: 7ms → 2ms, total 10ms ✓. Adding servers will not help (not a throughput issue). CPU upgrade minimal impact (not CPU-bound). Reducing volume defeats purpose. Always profile before optimizing - do not guess!',
  },
  {
    id: 'real-time-platform-project-mc-5',
    question:
      'Platform stores 50 billion ticks (2.5 TB compressed). Backup to S3 costs $0.023/GB. Monthly cost?',
    options: [
      '$58/month (2500 GB × $0.023)',
      '$575/month',
      '$23/month',
      '$230/month',
    ],
