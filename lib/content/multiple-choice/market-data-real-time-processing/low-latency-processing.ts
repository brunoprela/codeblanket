...
export const lowLatencyProcessingMultipleChoice: MultipleChoiceQuestion[] = [
  {
    ...
    question:
      'Python function takes 50μs. After Cython optimization, it takes 10μs. What is the speedup?',
    options: [
      '5× faster (50/10 = 5)',
      '40μs faster',
      '10× faster',
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation:
      'List scan complexity: O(n) = 1000 items × 50ns/comparison = 50,000ns = 50μs. Dict lookup: O(1) = 50ns regardless of size. Difference: 1000× slower for lists. At 10K lookups/sec: List = 500ms CPU (50%), Dict = 0.5ms CPU (0.05%). For order books: NEVER use lists (O(n) scan unacceptable). Use dict (O(1) lookup) or SortedDict (O(log n) = 10 comparisons = 500ns). Even O(log n) is 100× faster than O(n) for 1000 items.',
  },
  {
    id: 'low-latency-processing-mc-3',
    question:
      'Pre-allocated NumPy array (16 bytes/element) vs Python list (200 bytes/element). Memory savings for 10K elements?',
    options: [
      '92% savings (16 vs 200 bytes, 12.5× less)',
      '50% savings',
      '10× savings',
      'No savings',
    ],
  ...
  },
  {
    ...
    question:
      'CPU affinity pins trading thread to core 2. What is the benefit?',
    options: [
      'Eliminates context switching (10μs → 0μs)',
      'Faster CPU speed',
      'More memory',
      'Better network',
    ],
