...
export const pinterestarchitectureMultipleChoice: MultipleChoiceQuestion[] = [
  ...
  },
  {
    id: 'mc4',
    question:
      'How many shards does Pinterest typically use for object-based sharding?',
    options: ['256 shards', '1024 shards', '4096 shards', '16384 shards'],
    correctAnswer: 2,
    explanation:
      'Pinterest typically uses 4096 shards for object-based sharding. Shard ID is calculated as (object_id mod 4096). This provides fine-grained distribution to prevent hot shards while remaining manageable operationally. With 4096 shards, even users with millions of pins have their data distributed across many shards. Consistent hashing is used for adding/removing shards without full resharding.',
  },
  {
    id: 'mc5',
    question:
      'Which stream processing technology does Pinterest use for real-time user embedding updates?',
    options: [
    ...
    correctAnswer: 1,
    explanation:
      "Pinterest uses Apache Flink for real-time stream processing, including updating user embeddings when users save pins. When a user saves a pin, the event flows through Kafka to Flink, which updates the user's embedding vector in real-time. This updated embedding is immediately used for feed ranking, ensuring that the home feed reflects users' latest interests. Flink's stateful processing and low latency make it ideal for this use case.",
  },
];
