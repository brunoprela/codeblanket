...
export const slackarchitectureMultipleChoice: MultipleChoiceQuestion[] = [
  {
    id: 'mc1',
    question:
      'What protocol does Slack use for real-time messaging between clients and servers?',
    options: [
    ...
    correctAnswer: 2,
    explanation:
      'Slack uses WebSocket connections for real-time messaging with 30-second heartbeats to keep connections alive. Clients connect to Gateway servers which maintain persistent connections. When a user sends a message, it goes through the Gateway to Kafka (partitioned by channel_id), then back to Gateway servers which propagate to all connected clients in that channel. This provides low-latency message delivery (sub-100ms) at scale.',
  },
  {
    id: 'mc2',
  ...
  },
  {
    ...
    correctAnswer: 1,
    explanation:
      'Slack shards by workspace_id (tenant-based sharding). Each workspace is an independent unit, and most queries are within a workspace (no cross-shard joins needed). Large workspaces (>10,000 users) get dedicated shards, while small workspaces share shards. Vitess manages the MySQL sharding topology, and a routing service maps workspace_id to shard. This enables Slack to scale to millions of workspaces across thousands of MySQL shards.',
  },
  {
    id: 'mc4',
