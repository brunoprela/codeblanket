...
This system can detect ~80% of bankruptcies 12-18 months in advance while minimizing false positives to <20%.`
        ...
        {
            id: 2,
            question: "You're building a system to value companies based on liquidation value (worst-case scenario) versus going-concern value. Design a framework that automatically calculates both values from balance sheet data, including recovery rates for different asset classes. How would you handle intangible assets, goodwill, and contingent liabilities? When is liquidation value more relevant than going-concern value?",
            sample_answer: `A comprehensive dual-valuation system must accurately assess both liquidation (worst-case) and going-concern (normal operations) scenarios:
...
{
    id: 3,
        question: "You discover that a company's balance sheet shows $500M in 'Deferred Tax Assets' (DTAs). Explain what these represent, why they might be worth significantly less than book value, and design a system to automatically adjust book value for DTA quality. Under what circumstances are DTAs worthless, and how would you detect this from financial statements?",
            sample_answer: `Deferred Tax Assets are one of the most misunderstood and potentially misleading items on a balance sheet. A comprehensive DTA analysis system requires deep understanding of tax accounting:
