...
export const timeSpaceComplexityProblems: Problem[] = [
    // EASY #1 - Squares of a Sorted Array
    {
        id: 'squares-sorted',
...
**Challenge:** Can you do it in O(n) time?`,
        ...
        hints: [
            'Naive: square all elements then sort - O(n log n)',
            'Better: use two pointers at both ends - O(n)',
...
`,
        ...
        solution: `# Optimal: Two Pointers - O(n) time, O(n) space
...
# Naive: Square and sort - O(n log n) time
def sorted_squares_naive(nums):
    return sorted([num ** 2 for num in nums])
`,
        timeComplexity: 'O(n) optimal with two pointers vs O(n log n) with sorting',
        spaceComplexity: 'O(n) for output array in both approaches',
        leetcodeUrl: 'https://leetcode.com/problems/squares-of-a-sorted-array/',
    },

    // EASY #2 - Find Pivot Index
    {
        id: 'pivot-index',
...
Return the **leftmost pivot index**. If no such index exists, return \`-1\`.`,
        examples: [
            ...
            },
            {
                input: 'nums = [2,1,-1]',
                output: '0',
                explanation: 'The pivot index is 0. Left sum = 0. Right sum = 1 + (-1) = 0.',
            },
        ],
...
def pivot_index_naive(nums):
    ...
    return -1
`,
        timeComplexity: 'O(n) optimal vs O(n²) naive - demonstrates precomputation optimization',
...
def first_uniq_char_naive(s):
    ...
    return -1
`,
        timeComplexity: 'O(n) with hash map vs O(n²) naive - two passes vs nested loops',
...
`,
        ...
        spaceComplexity: 'O(1) optimal - demonstrates clever algorithm avoiding extra space',
...
**Key Insight:** To maximize the sum of minimums, pair adjacent elements after sorting!`,
        examples: [
            ...
            },
            {
                ...
                output: '9',
                explanation: 'Optimal pairing: (2, 1), (2, 5), (6, 6). Sum of mins = 1 + 2 + 6 = 9.',
            },
        ],
...
**Complexity Focus:** This problem tests your ability to implement a state machine with O(n) time and O(1) space.`,
        ...
        constraints: ['1 <= s.length <= 20', 's consists of only English letters (both uppercase and lowercase), digits (0-9), plus \'+\', minus \'-\', or dot \'.\'.'],
        hints: [
