...
def merge_sorted_arrays_v2(nums1, nums2):
    ...
    return result
`,
        timeComplexity: 'O(n + m) - we visit each element in both arrays exactly once',
        spaceComplexity: 'O(n + m) - we create a new array to store the merged result',
...
`,
        ...
        spaceComplexity:
            'O(1) for in-place solution, O(n) for creating new arrays',
...
`,
        ...
        spaceComplexity:
            'O(n) for auxiliary arrays during merge sort',
        leetcodeUrl:
