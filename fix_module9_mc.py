#!/usr/bin/env python3
"""
Fix Module 9 multiple choice files - properly format arrays
"""
...
# Write properly formatted multiple choice files
...
'''
    ...
    options: [
'''
        # Properly format each option on its own line
        for opt in q['options']:
            # Escape single quotes
            escaped_opt = opt.replace("'", "\\'")
            content += f"      '{escaped_opt}',\n"
...
print(f"\\nAll {len(mc_questions)} multiple choice files fixed!")
